...
export function mergeOffset(o){if(o.vehicleKind!=='bus'||!o.merge)return 0;const u=Math.max(0,Math.min(1,(o.z-55)/55));return (o.lane===0?-1:1)*8*u*u*(3-2*u);}
export function trafficAdvice(run){
 let nearest=null,eta=Infinity;for(const o of run.objects){if(o.type!=='bus'||o.hit)continue;const lead=(o.z-obstacleReach(o))/(run.speed+(o.approachSpeed||0));if(lead>0&&lead<4.5&&lead<eta){nearest=o;eta=lead;}}
