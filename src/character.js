...
export function createAriel(renderer,equipped){
 ...
 for(const [prefix,side] of [['Left',-1],['Right',1]])for(const kind of ['Arm','Leg']){
  ...
  chains.push({prefix,side,kind,upper,lower,end,l1:rest[upper].world.distanceTo(rest[lower].world),l2:rest[lower].world.distanceTo(rest[end].world),a:new T.Vector3(),target:new T.Vector3(),smooth:new T.Vector3(),dir:new T.Vector3(),pole:new T.Vector3(),mid:new T.Vector3(),bindAxis:new T.Vector3(),endAxis:new T.Vector3(),bindQ:new T.Quaternion(),deltaQ:new T.Quaternion(),initialized:false});
 }
 function local(v,x,y,z){return root.localToWorld(v.set(x,y,z));}
 function orient(name,x=0,z=0){const b=bones[name];b.parent.getWorldQuaternion(parentQ);rotationQ.setFromEuler(new T.Euler(x,0,z));desiredQ.copy(rootQ).multiply(rotationQ).multiply(rest[name].worldQuaternion);b.quaternion.copy(parentQ.invert()).multiply(desiredQ);b.updateWorldMatrix(false,true);}
 ...
 function solve(c,dt){
  ...
  if(!c.initialized){c.smooth.copy(c.target);c.initialized=true;}else c.smooth.lerp(c.target,1-Math.exp(-dt*22));
  c.dir.copy(c.smooth).sub(c.a);const original=c.dir.length(),dist=T.MathUtils.clamp(original,Math.abs(l1-l2)+.002,l1+l2-.002);c.dir.normalize();c.target.copy(c.a).addScaledVector(c.dir,dist);
  ...
  aim(c,c.upper,c.a,c.mid);aim(c,c.lower,c.mid,c.target);orient(c.end,c.kind==='Arm'?1.2:0);bones[c.end].getWorldPosition(worldV);errors[c.end]=worldV.distanceTo(c.target);targets[c.end]=root.worldToLocal(c.target.clone()).toArray();
 }
 ...
 return {root,bones,targets,errors,grips,soles,fsm,quality:assetQuality,avatar,draw(matrix,state,t){
  ...
  const dt=lastTime===null?1/60:Math.min(.1,Math.max(0,t-lastTime));lastTime=t;const updateDt=dt||1/60,pose=fsm.update(state,updateDt);
  ...
  hipDrop+=((slide?.53:hit?.24:0)+compress-hipDrop)*(1-Math.exp(-updateDt*22));
  ...
  const hips=bones.Hips;local(hip,rest.Hips.world.x,rest.Hips.world.y+rideBlend*.08-hipDrop+bob,rest.Hips.world.z+(slide?.08:0));hips.position.copy(hips.parent.worldToLocal(hip));hips.updateWorldMatrix(false,true);
  ...
  for(const c of chains){
   ...
   if(c.kind==='Arm'){
    if(riding){worldV.set(0,.078,0).applyQuaternion(rest[c.end].worldQuaternion).applyAxisAngle(axisX,1.2);local(c.target,side*.43-worldV.x,1.7-worldV.y,-.78-worldV.z);}
    ...
    else if(slide)local(c.target,side*.2,.16,side<0?-.16:.22);
    else if(jump||fly)local(c.target,side*.18,.30,side<0?-.15:.20);
    else local(c.target,side*.17,.16+(run?Math.max(0,s)*.24:0),run?s*.33:0);
   }
   solve(c,updateDt);
  }
