...
export class Run {
 ...
 add(type,lane,z,key,y){const o=this._free.pop();if(!o)throw Error('Límite de objetos de ruta superado');Object.assign(o,{id:this.nextId++,type,district:districtAt(this.distance+z,this.routeSeed),lane,z,key,y:y??(type==='coin'?1.15:type==='charge'?1.25:1.6),hit:false,passed:false,minDx:99,required:null,value:1,avoided:null,vehicleKind:null,length:0,approachSpeed:0,warned:false,merge:false});this.objects.push(o);this.poolPeak=Math.max(this.poolPeak,this.objects.length);return o;}
 ...
 const z=this.nextRow,safe=Math.max(0,Math.min(2,this.lastSafe+Math.floor(this.random()*3)-1));this.lastSafe=safe;
 // Authored rhythm: weave, compulsory jump, recovery, duck, rewarded risk.
 const patterns=this.distance<120?['weave']:['weave','jump','weave','duck','risk','rest'];let pattern=patterns[this.rowIndex%patterns.length];if(this.rowIndex%6===2&&this.random()>.5)pattern='rest';this.rowIndex++;
 ...
 if(this.distance>=this.nextEvent){const d=districtAt(this.distance,this.routeSeed),types={center:['police','Plaza del reloj · tráfico urbano'],residential:['park','Jardines · zona de descanso'],industrial:['crane','Grúa en movimiento · puerto'],metro:['station','Estación Ariel · tren en aproximación'],coast:['landmark','Faro del sur · paseo marítimo'],neon:['blackout','Distrito neón · luces intermitentes']};this.event={type:types[d][0],remaining:10,district:d};this.nextEvent+=1000;this.emit('city-event',{message:types[d][1]});}if(this.event){this.event.remaining-=dt;if(this.event.remaining<=0)this.event=null;}this.rain=this.distance%CONFIG.rainEvery>CONFIG.rainEvery-CONFIG.rainLength;this.wet+=(Number(this.rain)-this.wet)*(1-Math.exp(-dt*1.5));
 ...
 for(const o of this.objects){const oldZ=o.z;o.z-=travel+(o.approachSpeed||0)*dt;if(o.hit)continue;const cx=(o.lane-1)*CONFIG.laneWidth,dx=Math.min(Math.abs(cx-this.x),Math.abs(cx-prevX));const reach=obstacleReach(o);const near=o.z<reach&&oldZ>-reach;
