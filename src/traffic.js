...
export class Traffic {
 ...
 draw(distance,t,segments,low=false){this.active=0;const district=districtAt(distance,segments.seed),limit=low?10:20;for(let i=0;i<limit;i++){const o=this.slots[i],side=i%2?1:-1,parked=i%3===0,travel=parked?distance:distance+t*(i%2?8:-6),z=45-((travel+o.phase)%290);if(z>18||z<-250)continue;if(district==='coast'&&side>0)continue;this.vehicle(o.kind,side*(parked?7.1:9.2),0,z,side>0?Math.PI:0,t,parked?0:8);this.active++;}
  for(const s of segments.items)if(s.type==='intersection'&&s.z<-48&&s.z>-160){const x=Math.max(-36,Math.min(36,(s.z+92)*.86));this.vehicle(s.variant%2?'taxi':'car',x,0,s.z-10,Math.PI/2,t,6);}
  if(district==='metro'||district==='industrial'){const phase=t%34,u=Math.max(0,Math.min(1,(phase-4)/30)),z=55-310*(.5-.5*Math.cos(u*Math.PI));this.trainPositions[0]=z;this.train(-8,0,z,t);this.active+=3;if(district==='metro'){const elevated=40-((t*18+80)%360);this.trainPositions[1]=elevated;this.train(-8,5,elevated,t);this.active+=3;}}
 }
 obstacle(o,t){if(o.type!=='bus')return false;const laneX=(o.lane-1)*2.8,x=laneX+mergeOffset(o),z=-o.z;if(o.vehicleKind==='train'){this.train(x,0,z+(o.length||43)/2-7,t,Math.PI,3);if(o.z>30&&o.z<180)for(const side of[-1,1])this.lamp(Math.sin(t*8+side)>0?'red':'amber',laneX+side*1.25,2.3,z+o.length/2+3,0,2);}else this.vehicle(o.vehicleKind||'bus',x,0,z,Math.PI,t,o.approachSpeed||0,o.merge);return true;}
}
